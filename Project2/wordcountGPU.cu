...
__global__ void wordcount(char* words, int size, int n) {
...
int main(int argc, char *argv[]) {
  ...
  while(pos < filesize){
    long read = read(file, h_words, MEMORY_SIZE);
    pos += read;
    
    err = cudaMemcpy(d_words, h_words, read, cudaMemcpyHostToDevice);
    CHECK_ERR(err);
    
    wordcount<<< 1, BLOCK_SIZE >>>(d_words, read, ceil((((float)read)/((float)MEMORY_SIZE)) * BLOCK_SIZE)); 
